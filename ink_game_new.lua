...
local storyDropdown = main:CreateDropDown("Teleports", function() end)
...
main:CreateButton("Auto Beat Dalgona", function()
    local Players = game:GetService("Players")
...
main:CreateButton("Glass ESP", function()
    ...
    for i = 0, 11 do
        ...
        if not panel then
            warn("[Glass ESP] Missing panel: ClonedPanel" .. i)
        else
            for _, glassModelName in ipairs({"glassmodel1", "glassmodel2"}) do
                ...
                if glassModel then
                    ...
                    if glassPart and glassPart:IsA("BasePart") then
                        ...
                        if actuallyKilling == nil then
                            warn("[Glass ESP] 'ActuallyKilling' attribute missing in " .. glassPart.Name)
                            actuallyKilling = false
                        end

                        if actuallyKilling then
                            -- Unsafe glass
                        ...
                        warn("[Glass ESP] Missing or invalid 'glasspart' in " .. glassModelName)
                    end
                else
                    warn("[Glass ESP] Missing model: " .. glassModelName)
                end
            end
        end
    end
